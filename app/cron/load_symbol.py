...
def do_load():
    ...
    update_template = "update app_symbol set price_decimal=%s, amount_decimal=%s, created_at=%s, updated_at=%s where name='%s'"
    ...
    for i in data:
        try:
            ...
            sql = update_template % (i['price_decimal'], i['amount_decimal'], 'now()', 'now()', i['name']);
            cursor.execute(sql)
        # print(sql)
